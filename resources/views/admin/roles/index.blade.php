...
@section('content')
  <x-form>
    <div class="flex justify-between items-center mb-8">
      <x-form-header header="Fungsi" paragraph="Daftar fungsi / jabatan" />
      <a href="{{ route('admin.fungsis.create') }}" class="bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700">
        + Add Fungsi
      </a>
    </div>
      ...
      <tbody>
        @foreach ($fungsis as $fungsi)
          <tr class="border-t text-sm hover:bg-gray-50 transition">
            <td class="p-4 text-gray-600">{{ $fungsi->id }}</td>
            ...
            <td class="p-4">
              <span class="px-2 py-1 text-xs rounded
                                {{ $fungsi->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600' }}">
                {{ $fungsi->is_active ? 'Active' : 'Inactive' }}
              </span>
            </td>

            <td class="p-4">
              <div class="flex items-center gap-2">
                <a href="{{ route('admin.fungsis.show', $fungsi) }}"
                  class="px-3 py-1.5 text-sm rounded-md border border-sky-200 text-sky-600 hover:bg-sky-50">
                  Detail
                </a>

                <a href="{{ route('admin.fungsis.edit', $fungsi) }}"
                  class="px-3 py-1.5 text-sm rounded-md border border-amber-200 text-amber-600 hover:bg-amber-50">
                  Edit
                </a>

                <form action="{{ route('admin.fungsis.destroy', $fungsi) }}" method="POST"
